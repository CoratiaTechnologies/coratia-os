...
async def test_get_version() -> None:
    ...
    with mock.patch("builtins.open", mock.mock_open(read_data=SAMPLE_JSON)):
        ...
        assert result["repository"] == "coratia/coratiaos-core"
        assert result["tag"] == "coratia"
        assert len(client_mock.mock_calls) > 0


version = {"tag": "coratia", "image": "coratia/coratiaos-core", "pull": False}
...
async def test_set_version(write_mock: AsyncMock) -> None:
    ...
    with mock.patch("builtins.open", mock.mock_open(read_data=SAMPLE_JSON)):
        ...
        result = await chooser.set_version("coratia/coratiaos-core", "coratia")
